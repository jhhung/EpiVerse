...
class DictHiConformerDataGenerator(tf.keras.utils.Sequence):
    def __init__(self, config:dict, eval: bool = False, TF_permutation = False, seq_permutation=False, permutation_index=[]):
        self._config = AttrDict(config)
        self.eval = eval
        self.AVOCADO_assay_list = np.array(['ATAC-seq'           ,'ATF3'              ,'BHLHE40'                     ,'CAGE_minus'                 ,
        ...
        ###### Get permutation ###### 
        self.permutation_TF_case1 = TF_permutation
        self.seq_permutation = seq_permutation
        self.permutation_TF_index = permutation_index
        ###### Configuration ######
    ...
    def LoadTissueData(self):
        ...
        #self.Load_Single_Chrom_HiC_data("IMR90_MboI", "chr22")
        self.HiC_df = None
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            if self.seq_permutation:
                Seq_1 = np.zeros(shape=Seq_1.shape)
                Seq_2 = np.zeros(shape=Seq_2.shape)
    ...
    def Load_Single_Chrom_Avocado(self,Tissue:str,chrom:str,sample:str):
        ...
        #self.Avocado_track = np.sinh(self.Avocado_track)
        #print("[WARNING] Using sinh transformation for Avocado tracks!")
        #print("[WARNING] Using sinh transformation for Avocado tracks!")
        #print("[WARNING] Using sinh transformation for Avocado tracks!")
        ...
        if self._config.UseHighQualityTracks:
            ...
            if self._config.AvoRearrangement:
                ...
                self.Avocado_track = self.Avocado_track[:,mask]
                self.AVOCADO_assay_list = self.AVOCADO_assay_list[mask]
                if self.permutation_TF_case1:
                    print(self.permutation_TF_index)
                    print(self.AVOCADO_assay_list[self.permutation_TF_index])
                    print(f"Entering permuation TF mode (case 1), Now permutation {self.AVOCADO_assay_list[self.permutation_TF_index]}-index{self.permutation_TF_index}!")           
                    self.Avocado_track[:, self.permutation_TF_index] = shuffle(self.Avocado_track[:, self.permutation_TF_index])
        if self._config.AvoChromHMMOnly:
    ...
    def get_peakdiag_dfs(self, hic_df: pd.DataFrame, peak_df: pd.DataFrame, neg_ratio: float):
        ...
        if not self.eval:
            ...
            nonpeak_neg_idxs = sample(nonpeak_diag_idxs, int(num_pos_diags * neg_ratio))
